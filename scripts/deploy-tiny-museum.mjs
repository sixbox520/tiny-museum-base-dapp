...
const source = await readFile(resolve(process.cwd(), "contracts/TinyMuseum.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "TinyMuseum.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["TinyMuseum.sol"]?.TinyMuseum;
...
console.log(`Deploying TinyMuseum from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TINY_MUSEUM_CONTRACT_ADDRESS to this contract address in Vercel.");
